...
function ToggleButtonGroup({ value, options, onChange }) {
  return (
    <div className="inline-flex items-center rounded-[14px] bg-slate-50 p-1">
      {options.map((option) => {
        ...
        return (
          <button
            ...
            className={`rounded-[11px] px-4 py-2 text-sm font-semibold transition ${
              isActive
                ? "bg-white text-slate-900 shadow-[0_8px_20px_-14px_rgba(15,23,42,0.4)] ring-1 ring-slate-200"
...
function SortModeButton({ sortMode, onToggle }) {
  const label = sortMode === "priority" ? "중요도순" : "마감일순";

  return (
    <button
      type="button"
      onClick={onToggle}
      className="inline-flex items-center gap-2 rounded-[14px] border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-500 transition hover:border-slate-300 hover:text-slate-900"
    >
      <SortIcon />
      {label}
    </button>
...
function sortEventsChronologically(events) {
  return [...events].sort((firstEvent, secondEvent) => {
    if (firstEvent.start.getTime() !== secondEvent.start.getTime()) {
      return firstEvent.start - secondEvent.start;
    }

    if (firstEvent.end.getTime() !== secondEvent.end.getTime()) {
      return firstEvent.end - secondEvent.end;
    }

    return firstEvent.title.localeCompare(secondEvent.title, "ko");
  });
}
...
function sortTodosByDateAndPriority(todos) {
  const priorityRank = {
    high: 0,
    medium: 1,
    low: 2,
...
function CalendarPage() {
  ...
  const [eventViewMode, setEventViewMode] = useState("today");
  const [todoViewMode, setTodoViewMode] = useState("today");
  ...
  const visibleEvents = useMemo(() => {
    const filteredEvents =
      eventViewMode === "today"
        ? events.filter((event) => isWithinRange(today, event.start, event.end))
        : events;

    return eventViewMode === "today"
      ? sortEventsByAnchor(filteredEvents, today)
      : sortEventsChronologically(filteredEvents);
  }, [eventViewMode, events, today]);
        ...
        <div className="grid items-start gap-6 xl:grid-cols-[minmax(0,1.7fr)_390px]">
          ...
          <aside className="space-y-6">
            ...
            <section className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                ...
                <div className="flex flex-wrap items-center justify-end gap-3">
                  ...
                  <SortModeButton
                    ...
                    onToggle={() =>
                      setTodoSortMode((currentMode) =>
                        currentMode === "priority" ? "dueDate" : "priority",
                      )
                    }
                  />
                </div>
              ...
              {todoViewMode === "today" && completedTodos.length > 0 ? (
